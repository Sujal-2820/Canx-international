...
export function VendorRegister({ onSuccess, onSwitchToLogin }) {
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      ...
      const result = await vendorApi.requestVendorOTP({ phone: form.contact })
  ...
  const handleVerifyOtp = async (otpCode) => {
    ...
    try {
      ...
      if (result.success || result.data) {
        ...
        onSuccess?.(result.data?.vendor || { name: form.fullName, phone: form.contact })
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-green-50 via-white to-green-50 px-6 py-12">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs uppercase tracking-wide text-green-600 font-semibold">Vendor Registration</p>
          <h1 className="text-3xl font-bold text-gray-900">Join as Vendor</h1>
          <p className="text-sm text-gray-600">Start managing your fertilizer distribution</p>
        </div>
        ...
        <div className="rounded-3xl border border-green-200/60 bg-white/90 p-8 shadow-xl backdrop-blur-sm">
          <form onSubmit={handleRequestOtp} className="space-y-5">
            ...
            <div className="space-y-1.5">
              <label htmlFor="vendor-register-fullName" className="text-xs font-semibold text-gray-700">
                Full Name <span className="text-red-500">*</span>
              </label>
              <input
                id="vendor-register-fullName"
                name="fullName"
                type="text"
                required
                value={form.fullName}
            ...
            <div className="space-y-1.5">
              <label htmlFor="vendor-register-contact" className="text-xs font-semibold text-gray-700">
                Contact Number <span className="text-red-500">*</span>
              </label>
              <input
                id="vendor-register-contact"
                name="contact"
                type="tel"
                required
                value={form.contact}
