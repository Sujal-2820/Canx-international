/**
 * Vendor Price Display Component
 * 
 * Shows vendor-specific pricing with discounts
 * Uses discountVendor from product model
 */

import { cn } from '../../../lib/cn'
import { Trans } from '../../../components/Trans'

export function VendorPriceDisplay({
    originalPrice,
    discount = 0,
    quantity = 1,
    size = 'md',
...
export function VendorCartPriceSummary({ items = [], className = '' }) {
    const subtotal = items.reduce((sum, item) => {
        const itemPrice = item.price || item.priceToVendor || 0
    ...
    const totalDiscount = items.reduce((sum, item) => {
        const itemPrice = item.price || item.priceToVendor || 0
        const itemDiscount = item.discount || item.discountVendor || 0
        const itemQty = item.quantity || 1
