                ...
                <div className="absolute inset-0 z-[110] flex items-center justify-center text-white text-center p-10 bg-black/40 backdrop-blur-sm">
                    <div className="space-y-4 max-w-xs animate-in zoom-in duration-300">
                        ...
                        <p className="text-xs font-medium text-white/50 leading-relaxed">
                            To view this invoice, please ensure the window remains in focus. Screenshots and external recordings are prohibited by Canx Security Protocol.
                        </p>
                    </div>
                </div>
            )}
                ...
                <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-[0.04] rotate-[-25deg] flex flex-wrap gap-x-24 gap-y-32 p-10 select-none preserve-3d">
                    {Array.from({ length: 60 }).map((_, i) => (
                        <span key={i} className="text-3xl font-black whitespace-nowrap tracking-tighter uppercase italic">
                            CANX INT SECURE • {new Date().toLocaleDateString()} • CONFIDENTIAL
                        </span>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 relative z-10">
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em]">Authorized Seller</p>
                        </div>
                        <div className="space-y-1.5">
                            <h3 className="text-base font-black text-gray-900 tracking-tight">CANX INTERNATIONAL</h3>
                    ...
                    <div className="text-right space-y-3">
                        ...
                        <div className="space-y-1.5">
                            <h3 className="text-base font-black text-gray-900 tracking-tight uppercase">
                                {vendor?.shopName || vendor?.name || 'Authorized Partner'}
                            </h3>
                            <p className="text-[11px] text-gray-500 leading-relaxed font-bold">
                                {vendor?.location?.address}<br />
                                {vendor?.location?.city}, {vendor?.location?.state}<br />
                                Pin: {vendor?.location?.pincode}
                            </p>
                            <div className="pt-1 flex flex-col items-end gap-0.5">
                                <p className="text-[10px] font-black text-gray-900 border-r-2 border-gray-200 pr-2 uppercase">VNDR-ID: {vendor?._id?.slice(-8).toUpperCase()}</p>
                                <p className="text-[10px] font-bold text-gray-400 pr-2">TEL: +91 {vendor?.phone}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-16 pb-10 text-center space-y-6 relative z-10">
                    <div className="inline-flex items-center gap-3 px-6 py-2 bg-white rounded-2xl border-2 border-gray-100 shadow-sm transform -rotate-2">
                        ...
                        <span className="text-[10px] font-black uppercase italic tracking-tighter text-gray-900 opacity-60">
                            CERTIFIED FOR {vendor?.name || 'VNDR'} • SECURE-INV-{(order._id || 'XXX').slice(0, 8).toUpperCase()}
                        </span>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-center gap-1.5 text-[8px] font-black text-blue-600 uppercase tracking-widest">
                            ...
                            CANX GLOBAL SECURITY COMPLIANT
                            <div className="h-1 w-1 bg-blue-400 rounded-full"></div>
                        </div>
                    </div>
                </div>
