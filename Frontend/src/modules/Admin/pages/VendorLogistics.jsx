...
export function VendorLogisticsPage({ navigate }) {
    const {
    ...
    const fetchPurchases = useCallback(async () => {
        ...
        console.log('[VendorLogistics] Fetching purchases with params:', params)
        const result = await getVendorPurchaseRequests(params)
        console.log('[VendorLogistics] API Result:', result)

        if (result.success && result.data?.purchases) {
            console.log('[VendorLogistics] Purchases received:', result.data.purchases.length)
            ...
            console.log('[VendorLogistics] After filtering:', list.length)
            setPurchases(list)
        } else {
            console.error('[VendorLogistics] Failed to fetch purchases:', result.error)
            setPurchases([])
        }
    }, [getVendorPurchaseRequests, filters])
    ...
    const handleProcessOrder = async (id) => {
        const res = await processVendorPurchaseStock(id, { deliveryNotes: notes || 'Order is being processed and packed.' })
        if (res.success) {
    ...
    const handleDispatchOrder = async (id) => {
        const res = await sendVendorPurchaseStock(id, { deliveryNotes: notes || 'Stock has been dispatched.' })
        if (res.success) {
    ...
    const handleConfirmDelivery = async (id) => {
        const res = await confirmVendorPurchaseDelivery(id, { deliveryNotes: notes || 'Stock delivered and verified.' })
        if (res.success) {
    ...
    const tableData = purchases.map(p => ({
        ...p,
        id: p.creditPurchaseId || p.requestId || p.id,
        vendorName: p.vendor?.name || 'Unknown',
        location: p.vendor?.location?.city || p.vendor?.region || 'Unknown',
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex flex-col sm:flex-row justify-between gap-4 p-4 rounded-xl bg-blue-50/50 border border-blue-100">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <StatusBadge tone="neutral">{p.paymentMode || 'Credit Line'}</StatusBadge>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {p.status === 'approved' && p.deliveryStatus !== 'delivered' && (
                            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm overflow-hidden relative">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Delivery Notes (Internal/Vendor visible)</label>
                                        <textarea
                                            value={notes}
                        ...
                        {p.deliveryStatus === 'delivered' && (
                            <div className="rounded-2xl border border-green-200 bg-green-50 p-6 text-center">
                                ...
                                <p className="text-sm text-green-700 mt-1">This stock request has been successfully delivered and added to vendor's inventory.</p>
                                {p.deliveredAt && (
                    ...
                    <div className="space-y-6">
                        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">Vendor Info</h3>
                            <div className="space-y-4">
                                <div className="flex gap-4">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-gray-900">{p.vendorName}</p>
                                        <p className="text-xs text-gray-500">{p.vendor?.phone || 'No phone'}</p>
                                    </div>
                                </div>
                                <div className="flex gap-4">
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-gray-900">Vendor Logistics</h1>
                    <p className="text-sm text-gray-500">Manage stock dispatch and delivery for vendor purchase requests.</p>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
                <div className="relative flex-1 w-full">
                    ...
                    <input
                        type="text"
                        placeholder="Search Vendor name or Request ID..."
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <DataTable
                    ...
                    rows={tableData}
                    loading={loading}
                    emptyMessage="No matching purchase requests found for logistics."
                />
            </div>
        </div>
    )
