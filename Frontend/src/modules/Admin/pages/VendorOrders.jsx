...
export function VendorOrdersPage({ navigate }) {
    const {
    ...
    const fetchOrders = useCallback(async () => {
        setRefreshing(true)
        const result = await getVendorPurchaseRequests({ status: 'approved' })
    ...
    const handleUpdateStatus = async (orderId, action) => {
        ...
        if (action === 'processing') {
            result = await processVendorPurchaseStock(orderId, { deliveryNotes: 'Order is being processed.' })
        } else if (action === 'dispatch') {
            result = await sendVendorPurchaseStock(orderId, { deliveryNotes: 'Stock has been dispatched.' })
        } else if (action === 'delivered') {
            result = await confirmVendorPurchaseDelivery(orderId, { deliveryNotes: 'Stock delivered successfully.' })
        }
    ...
    const tableData = orders.map(order => ({
        ...
        vendorName: order.vendorId?.name || order.vendor?.name || 'Unknown',
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-xs uppercase tracking-wide text-gray-500 font-semibold">Vendor Management</p>
                    <h2 className="text-2xl font-bold text-gray-900">Vendor Orders</h2>
                    <p className="text-sm text-gray-600">
                        Manage delivery status for approved vendor credit purchases
                    </p>
                </div>
                <button
                    onClick={fetchOrders}
            ...
            {orders.length === 0 ? (
                <div className="rounded-3xl border border-gray-200 bg-white p-12 text-center">
                    ...
                    <p className="mt-4 text-sm font-bold text-gray-900">No Vendor Orders</p>
                    <p className="mt-2 text-xs text-gray-600">
                        {refreshing ? 'Loading...' : 'No approved vendor orders found.'}
                    </p>
                </div>
            ) : (
